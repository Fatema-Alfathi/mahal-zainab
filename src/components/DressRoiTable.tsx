...
export function DressRoiTable() {
  ...
  return (
    <section>
      <div className="mb-5">
        <p className="text-sm text-rose-400">{t("roi.kicker")}</p>
        <h3 className="mt-1 text-2xl font-medium text-rose-900 sm:text-3xl">{t("roi.title")}</h3>
            ...
            <article key={dress.id} className="shop-card rounded-3xl p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {brokenEven ? (
                    ...
                    <p className="mt-1 text-xs text-rose-300">{t("roi.notYet")}</p>
                  )}
                </div>
              ...
              <dl className="mt-4 grid grid-cols-2 gap-2 text-sm sm:grid-cols-3">
                <div className="rounded-2xl bg-rose-50 px-3 py-2">
                  <dt className="text-[11px] text-rose-400">{t("roi.cost")}</dt>
                  <dd className="mt-0.5 tabular-nums text-rose-900">{formatCurrency(landed)}</dd>
                </div>
                <div className="rounded-2xl bg-amber-50 px-3 py-2">
                  <dt className="text-[11px] text-amber-600">{t("roi.earned")}</dt>
              ...
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-[11px] text-rose-400">
                  <span>{brokenEven ? t("roi.brokeEven") : t("roi.recovered")}</span>
                  <span className="tabular-nums">{t("disc.percentValue", { value: Math.round(barWidth) })}</span>
                </div>
